        ...
        if (not(isTackle)):
            ...
            if (props.diamonds == props.inventory_size) or (steps_to_base == time_left+1):
                ...
                if (isTele == -1):
                    ...
                    if not(position_equals(telePos, current_position)):
                        self.goal_position = list_Teleport.pop(isTele).position
                    else:
                        self.goal_position = base
            ...
            # Algo Searching for Goal
                ...
                if (pair_dia_dist != []):
                    ...
                    min_distance_elem = sorted(pair_dia_dist, key=lambda elem: ((elem[0].properties.points / elem[1]), -(elem[1])), reverse=True)[0]
